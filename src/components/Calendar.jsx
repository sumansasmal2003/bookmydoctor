...
const Calendar = ({ weekStart, appointments, onSlotClick, onAppointmentClick }) => {
  ...
  return (
    <div className="max-w-full mx-auto bg-white dark:bg-gray-800 rounded-xl shadow-xl overflow-hidden border border-gray-100 dark:border-gray-600">

      {/* Main Grid Section */}
      <div className="flex">
        {/* Left Time Column - Sticky */}
        <div className="w-20 md:w-24 flex-shrink-0 sticky left-0 bg-gray-50 dark:bg-gray-700 border-r border-gray-100 dark:border-gray-600">
          <div className="h-16 flex items-center justify-center bg-gradient-to-b from-gray-50 to-gray-75 dark:from-gray-700 dark:to-gray-600 border-b dark:border-gray-600">
            <span className="text-sm font-semibold text-gray-500 dark:text-gray-300 tracking-wide">TIME</span>
          </div>
          {timeSlots.map(hour => (
            <div
              key={hour}
              className="h-16 md:h-20 border-b border-gray-100 dark:border-gray-600 flex items-center justify-center text-xs md:text-sm font-medium text-gray-500 dark:text-gray-300"
            >
          ...
          <div className="grid grid-cols-7 min-w-[800px]">
            ...
            {days.map((date) => (
              <div
                ...
                className="h-16 border-b border-l border-gray-100 dark:border-gray-600 text-center p-2 sticky top-0 bg-gradient-to-b from-gray-50 to-gray-75 dark:from-gray-700 dark:to-gray-600"
              >
                <div className="text-xs uppercase tracking-wide text-gray-500 dark:text-gray-300 font-medium mb-1">
                  {date.format('ddd')}
                </div>
                <div className={`text-lg font-semibold ${
                  date.isSame(dayjs(), 'day')
                    ? 'text-blue-600 dark:text-blue-300 bg-blue-100 dark:bg-blue-900 w-8 h-8 rounded-full mx-auto flex items-center justify-center'
                ...
                {days.map((date) => {
                  ...
                  return (
                    <div
                      ...
                      className="h-16 md:h-20 border-b border-l border-gray-100 dark:border-gray-600 relative group hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer"
                    ...
                    >
                      ...
                      {appointmentsInHour.length === 0 && (
                        <div className="absolute inset-0 flex items-center justify-center opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                          <div className="p-1.5 bg-white dark:bg-gray-800 rounded-full shadow-sm">
                            <FaPlus className="w-5 h-5 text-blue-600 dark:text-blue-300" />
                          </div>
                        </div>
                      )}
                      ...
                      {showIntro && hour === 9 && date.isSame(weekStart, 'day') && (
                        <div className="absolute -top-8 md:-top-15 left-20 -translate-x-1/2 w-[160px] bg-purple-600 text-white text-xs font-medium rounded-md px-3 py-2 shadow-lg z-10 animate-bounce">
                      ...
                      <div className="absolute inset-0 p-1 space-y-px overflow-hidden flex flex-col justify-center">
                        {appointmentsInHour.map(appt => {
                          ...
                          return (
                            <div
                              key={appt.id}
                              className={`group/event relative rounded-xl bg-gradient-to-r ${colorClass} shadow-xs hover:shadow-sm transition-all cursor-pointer overflow-hidden`}
                              onClick={(e) => {
                                e.stopPropagation();
      ...
      <div className="md:hidden py-3 text-center text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700 border-t border-gray-100 dark:border-gray-600">
